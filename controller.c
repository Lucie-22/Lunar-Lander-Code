...
void *dashboard(void *data)
{
	...
	ssize_t sendto(int d, const void *buffer, size_t bufsize, int flags, const struct sockaddr *daddr, socklen_t len);
...
{
// open file, logFile = fopen("dataLog,txt", "w");

// if error opening file, print error
/* if (logFile == NULL) {
	fprintf (stderr, "ERROR OPENING FILE");
{
*/

// fwrite() or fputc() to add the data to the file
...
int main(int argc, char *argv[])
{
    ...
    if ((e = pthread_create(&dash, NULL, dashboard, argv[1])))
    	fprintf(stderr, "not created dashboard threads: %s\n", strerror(e));
